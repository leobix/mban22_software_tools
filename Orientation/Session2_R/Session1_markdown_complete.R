...
#' 
#' 
#' Supervised learning refers to those tasks in which we observe some input $x$ and use it to predict some output $y$. When the target output $y$ is continous we refer to the problem as **regression**, and when the label is instead categorical, we call it a **classification** problem. 
#' 
#' Unsupervised learning, instead, refers to tasks in which we aim to understand and analyze the input $x$ without any target $y$. **Clustering**, for example, is an unsupervised task that groups data points with similar characteristics.
#' 
...
#' 
#' 
#' Given a dataset $\mathcal{D} = \{ (x_i, y_i) \}_{i = 1}^{n}$, where $y_i \in \mathbb{R}$ is the desired output for the input $x_i\in \mathbb{R}^d$, the goal of a regression task is to use the dataset to find a function $h$ such that $h(x)$ yields an accurate approximation of the desired output $y$ for any unseen input $x$.
#' 
#' The function $h$ is usually found by selecting a family of models (e.g. linear models, tree models, neural networks, etc.), and solving an optimization problem to find the best model within this family according to some *loss function*. 
#' 
...
#' 
#' 
#' In our example, we're going to use the Boston Airbnb dataset, where each entry is a property listing. The goal is to use the description of each listing (the $x$) in order to predict its price (the $y$).
#' 
...
#' 
#' To have a well defined input $x\in \mathbb{R}$, we need the variables (columns) in it to be interpreted as numbers. For this, we need to convert all continous variables to have numeric type and all categorical variables to have factor type.
#' 
#' We can use the data wrangling tools learned earlier to pre-process our data and solve all these issues. We have done that in the "process_listings.R" file, which you can open and see in detail. In this session, we will simply call the function process_listings() to obtain the already processed data set:
...
#' 
#' Note that there are many variables for each listing in the dataset, and it's our job to choose which ones we will include in the input $x_i$. This choice is part of the modeling process, and we will iterate on it.
#' 
...
#' 
#' 
#' The first family of models we're going to try is linear models. This means we hypothesise that the output $y$ can be described using a linear combination of the inputs, $h(x) = w^{\intercal} x$.
#' 
...
#' 
#' NOTE:  The loss function is used to fit a model, and the measure of accuracy is used to evaluate how well it explains the data. The measure of accuracy usually comes from the task we aim to solve, and the loss function is often something that we will play around with until we find a good model. 
...
#' 
#' When building the OLS model, we used the entire dataset. Simply taking the R squared value on this dataset as a measure of performance is clearly not fair -- small residuals in the training set do not guarantee a small residual for a new observation ($x$, $y$).
#' 
#' 
...
#' 
#' The validation set is like an 'intermediate' estimate of performance. If we didn't use the validation set for this purpose, the only way of selecting the best model from a model class would be to look at its performance on the testing set, and therefore we would be kinda of "cheating" by using the "unseen" data to select the model.
#' 
...
#' 
#'
#' Logistic regression is part of the class of generalized linear models (GLMs), which build directly on top of linear regression. These models take the linear fit and map it through a non-linear function. Linear regression has the form $$y_i= w^Tx_i,$$ whereas a GLM model has the form $$y_i=f(w^Tx_i)$$ For logistic regression, the function $f()$ is given by $f(z) = 1/(1+\exp(-z))$  (the *logistic function*), which looks like this:	
...
#' $\min_w \: \frac{1}{n} \sum_{i = 1}^{n} (w^{\intercal} x_i - y_i)^2$
#' 
#' And we heard that the optimal $w^{*}$ can be written in closed form. If we tried to minimize mean squared error for logistic regression, we would *not* be able to obtain $w^{*}$ easily. Intuitively, we also might want a different loss function. We want to penalize misclassification (e.g. classifying something that is not spam as spam). 
#' 
...
#' 
#' Let's apply this model to the `listings` data. Let's try to predict which listings have elevators in the building by using `price` as a predictor. This information is contained in the `amenities` column. 
#' 
...
#' An indicator for each amenity is now stored in a column called `amentity_x` where `x` is the amenity of interest. Now back to our task of predicting which listings have elevators. To make sure we're asking a sensible question, we'll only consider apartments priced at $500 or less. 
...
#'We will use the command `sample.split` from the `caTools` library to split the data. One nice thing about using `sample.split` for classification is that it preserves the ratio of class labels in the training and testing sets.
...
#' Notice that we could also create a validation set as we did in regression to choose the best model. However, we are going to skip the validation part for this section since it is pretty similar to what we did before. Instead, we will wait to cover trees to explore a new validation strategy.
#'
...
#' 
#' The *true positive rate* is the number of true positives divided by the total number of positives, and the *false positive rate* is the numbe of false positives divided by the total number of negatives.)	
#' 
#' 
#' 
...
#' As you can see, the `performance()` function in the `ROCR` package is versatile and allows you to calculate and plot a bunch of different performance metrics. In our case, this model gives an AUC of 0.68. 
#' 
...
#' 
#' Notice that in linear regression and logistic regression, the only decision we need to make is which columns to include in our input variables $x$. CART however, has many parameters that specify how the decision tree is constructed, and one of the most important is `cp`, the complexity parameter.
#'
...
#'
#' 
#' We will briefly take a look at random forests, using the `randomForest` package. A random forest is a collection of slightly randomized decision trees (hence the name "forest"), and can be used for classification or prediction. 
#' 
#' They often have excellent predictive performance, but can be expensive to train and lack interpretability. Random forests have many hyperparameters that can be tuned to achieve the best possible predictive performance. Perhaps the most important hyperparameter is the number of trees to include in the forest. 
#' 
...
#' Let's start by training a random forest model for a classification task. We will perform the same task of predicting whether or not a listing has an elevator, using price and neighborhood as predictors. 
#' 
...
#' The prediction accuracy is now much better, and it doesn't look we are overfitting at all. It would likely be even better if the other hyperparameters of the random forest model were properly tuned! 
#' 
#' Although random forest models are not very interpretable and hard to visualize, there is a popular method called *variable importance* that is commonly used with random forest models. We will use the function `varImpPlot` form the `randomForest` package.
#' 
...
#'
#' 
#' Thus far, our machine learning task has been to predict labels, which were either continuous-valued (for regression) or discrete-valued (for classification).  To do this, we input to the ML algorithms some known (feature, label) examples (the training set), and the ML algorithm outputs a function which enables us to make predictions for some unknown examples (the testing set).
#' 
#' 
